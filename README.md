...
This repository will be kept up to date with new Multiwfn releases. It is currently compatible with version 2026.7.11.
...
1. The Makefile will auto-detect the compiler in your system environment. If you have Intel Fortran compiler, it will be used; if Intel Fortran compilers are not detected, then gfortran will be used. You can also force the compiler by adding `COMPILER=gnu` or `COMPILER=intel`.
...
5. By default, the build uses `ARCH=generic`, which targets the SSE4.2 instruction set to maintain binary compatibility and portability. If you would like to compile with the maximum instruction set supported by your local CPU, you can use `ARCH=native`.
...
这里是我自己尝试性为[Multiwfn](http://sobereva.com/multiwfn/)程序做的一个Makefile系统，包含主Makefile和arch目录下的两个makefile.include，分别使用gfortran和ifx编译器。这一尝试旨在使得Multiwfn能够顺利利用gfortran进行编译。如果你想尝试利用这里的Makefile自行编译Multiwfn，直接将Makefile和config目录复制到源代码目录下，然后运行`make -jN`即可。具体可用选项可以通过`make help`看到。

关于`make`的具体使用，有以下几点：
1. Makefile默认自动检测编译器，规则如下：如果存在Intel Fortran编译器（即ifx），优先使用Intel的；如果不存在Intel编译器，则使用gfortran。你也可以通过在`make`命令后添加`COMPILER=gnu`或`COMPILER=intel`强行指定你想要的编译器。
...
5. 默认编译使用`ARCH=generic`，即使用SSE4.2指令集，以保持二进制文件的兼容性和可移植性。想要利用本地CPU支持的最大指令集进行编译，你可以加`ARCH=native`。
