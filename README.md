...
4. By default, the build uses `TYPE=generic`, which targets the SSE3 instruction set (as in the original Makefile) to maintain binary compatibility and portability. If you would like to compile with the maximum instruction set supported by your local CPU, you can use `TYPE=native`.
...
4. 默认编译使用`TYPE=generic`，即使用SSE3指令集（与原始Makefile一致），以保持二进制文件的兼容性和可移植性。想要利用本地CPU支持的最大指令集进行编译，你可以加`TYPE=native`。
